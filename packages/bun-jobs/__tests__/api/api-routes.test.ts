...
describe("mounting", () => {
  ...
  it("reports the routes it registered", () => {
    ...
    expect(described).toEqual([
      ...
      "GET /admin/jobs/overview/added getAddedByState metrics.read",
      "GET /admin/jobs/queues listQueues queues.list",
      "GET /admin/jobs/demand listQueueDemand queues.list",
      ...
      // Saving and applying job defaults are opt-in, so only the read is here.
      "GET /admin/jobs/queues/:queue/job-defaults getJobDefaults queues.read",
      ...
      "GET /admin/jobs/queues/:queue/workers/:worker getWorker workers.read",
      "POST /admin/jobs/queues/:queue/workers/:worker/pause pauseWorker workers.pause (mutation)",
      "POST /admin/jobs/queues/:queue/workers/:worker/resume resumeWorker workers.resume (mutation)",
      "POST /admin/jobs/queues/:queue/workers/:worker/stop stopWorker workers.stop (mutation)",
      "POST /admin/jobs/queues/:queue/workers/:worker/start startWorker workers.start (mutation)",
      "GET /admin/jobs/queues/:queue/worker-configs listWorkerConfigs workers.read",
      ...
      "DELETE /admin/jobs/queues/:queue/jobs/:id/logs clearJobLogs jobs.clearLogs (mutation)",
      ...
      "POST /admin/jobs/queues/:queue/jobs/:id/fail failJob jobs.fail (mutation)",
      ...
      "POST /admin/jobs/queues/:queue/repeatables/:key/disable disableRepeatable repeatables.disable (mutation)",
      "POST /admin/jobs/queues/:queue/repeatables/:key/enable enableRepeatable repeatables.enable (mutation)",
      ...
      "DELETE /admin/jobs/runners/:runner/history clearRunnerHistory runners.clearHistory (mutation)",
      "GET /admin/jobs/runners/:runner/runs/:runId/logs getRunLogs runners.logs",
      ...
      "GET /admin/jobs/analytics/jobs getJobsAnalytics metrics.read",
      "GET /admin/jobs/queues/:queue/analytics/jobs getQueueJobsAnalytics metrics.read",
      "GET /admin/jobs/analytics/runners getRunnersAnalytics metrics.read",
      "GET /admin/jobs/runners/:runner/analytics getRunnerAnalytics metrics.read",
      "GET /admin/jobs/analytics/workers getWorkersAnalytics metrics.read",
      "GET /admin/jobs/queues/:queue/analytics/workers/:key getWorkerAnalytics metrics.read",
    ]);
  });
...
describe("GET /meta", () => {
  ...
  it("describes the API and its backend, matching the Meta schema", async () => {
    ...
    expect(body).toEqual({
      ...
      // driver serving none answers `null`, never an absent key (asserted in
      // api-analytics.test.ts).
      analytics: {
        resolutions: [1, 60],
        retentionMs: { 1: 300_000, 60: 86_400_000 },
        maxSpanMs: 86_400_000,
        maxBuckets: 1500,
        maxSeries: 20,
        recording: {
          resolution: "second",
