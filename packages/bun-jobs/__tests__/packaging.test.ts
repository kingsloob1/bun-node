 ...
 * (`bun scripts/consumer-check.ts packages/bun-jobs` from the repo root),
...
describe("build-declarations rewrite", () => {
  it("makes relative specifiers explicit and leaves bare ones alone", async () => {
    const dir = await mkdtemp(join(tmpdir(), "bun-jobs-dts-"));
    try {
...
describe("this package's optional peers", () => {
  ...
  // mongodb and chrono-node are loaded at runtime through a `string`-typed
  // specifier and described by local structural types, so no entry needs
  // either. A declaration importing one would fail the build's verify step.
  it("keeps mongodb and chrono-node structural: no entry needs a peer", () => {
    expect(optional).toEqual(
      new Set(["@types/bun", "chrono-node", "mongodb", "typescript"]),
    );
    for (const entry of checkConfig.entries ?? [])
      expect(entry.peers ?? []).toEqual([]);
  });

  it("checks the contract entry as browser-safe", () => {
    const contract = checkConfig.entries?.find(
      (e) => e.spelling === "@kingsleyweb/bun-jobs/api/contract",
