...
| Runner history pager | the Runner stats and history's needs, and the runner has stored more runs than one page holds — the “Runs shown” number, which is the page size. It pages the runner's whole history **on the server**, re-reading it with a new offset, and keeps the window in the URL; a run whose log the URL opens is on the page that link carries, and the card says so when it is not on the page shown |
