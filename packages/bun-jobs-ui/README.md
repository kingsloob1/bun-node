...
| `/queues/:queue` | `panel` | The open detail panel: `limits`, `job-defaults`, `workers`, `demand`, `throughput` or `repeatables`. Defaults to the first one shown. |
...
| `/runners` | `search` | Filters by id or name (case-insensitive, trimmed), as you type. The list is filtered in the browser, so nothing is re-fetched. Changing it resets `offset`. |
| `/runners` | `offset` | Runners skipped, over the runners the filter left. Defaults to `0`. The list is paged in the browser, so no request carries it; a value past the last page shows the last page with rows. |
| `/runners` | `limit` | Runners per page. Defaults to `25`, and the select offers `10`, `20`, `25`, `50` and `100`. |
| `/runners/:runner` | `logs` | The run id whose log is open, e.g. `logs=run-3`. The run's row in the history is expanded with it, and its log read. A Log button writes it into the query string it was pressed on, so a copied link carries `offset` too and opens on the page holding that run. An id no run on the page has opens nothing, and the card says the open log is elsewhere, with a Close log. Absent means no log is open, and nothing is read. |
...
| `/runners/:runner` | `history` | Runs **per page** in the history, sent as `GET /runners/:runner/history?limit=`. Defaults to the smaller of `50` and `limits.maxHistory`. A number outside `1` to `limits.maxHistory` is clamped to that range, not reset. The "Runs shown" select and the pager's own "Rows per page" are two views of it, offering `10`, `25`, `50`, `100` and `200` up to the cap, plus the default and the cap. Choosing the default removes the parameter, and any change resets `offset`. |
| `/runners/:runner` | `offset` | Runs skipped before the page, sent as `GET /runners/:runner/history?offset=`. Defaults to `0`. Deliberately uncapped, as the route is: `keepHistory` may store far more runs than `limits.maxHistory`, and this is what reaches them. A value past the end answers an empty page — "No runs on this page", with the pager and Clear history… still live — rather than the last page with rows. |
...
| Overview queue table pager | the Overview queue table's needs, and more queues came back than one page holds (20). The card cuts its pages in the browser from what the queue list returned, so turning a page reads nothing, and a table that fits one page grows no pager |
...
| Overview queue table Demand column | the Overview queue table's needs and `features.demand`. One `GET /demand` for the queues on the page shown, not per row; a queue the answer leaves out shows "—". Each figure is written as in the Demand panel and links to it |
...
| Overview Runners section row links | a row's runner links to its runner page where those pages are routed for the caller (the Runners nav entry's needs, read on the untargeted map); plain text otherwise. A row is built from what was recorded in the range, so it can name a runner the API no longer knows — a fleet host reporting counts for ids it holds no record of — and the link then lands on the runner screen's own "no longer exists" state. Note that `runners.remove()` does not produce that: a removed runner stays in the registry and still answers `GET /runners/:id` |
...
| Demand panel | `queues.read` and `features.demand`, which is true on every backend and false only in `runner` mode. Each figure as its answer qualifies it: "≥" on every count when the answer is `capped`, since the API does not say which count reached the cap, and "≈" on demand, outstanding, due now and stalled when it is not `exact`, never on waiting or active. A note says which applies, and a paused queue gets a note that its 0 is not an empty queue |
...
| Repeatables panel pager | the Repeatables panel's needs, and the queue has more repeat series than one page holds (20) |
...
| Runners list pager | the Runners nav entry's needs, and more runners match the filter than one page holds (25 by default; `limit` is in the URL, so the size a reader picks is what a page holds). Its window lives in the URL, as the queue list's does, and changing the filter restarts it |
...
| Runner history pager | the Runner stats and history's needs, and the runner has stored more runs than one page holds — the “Runs shown” number, which is the page size (50 by default, or `limits.maxHistory` when that is smaller). It pages the runner's whole history **on the server**, re-reading it with a new offset, and keeps the window in the URL; a run whose log the URL opens is on the page that link carries, and the card says so when it is not on the page shown |
...
| Runner Clear history… | mutation `runners.clearHistory`, on any runner: unlike Kill… and Reset stats…, a runner registered only in another process gets it too, and it does not bring up the non-local hint; it sits in the History card's header, and is disabled, with "No runs to clear.", while the history is empty |
| Runner non-local hint | mutation `runners.kill` or `runners.resetStats`, and the runner is not local |
...
| Workers table target badge (the Workers page, a queue's Workers panel and a worker page's Instances table) | the worker reports `target`; a worker too old to report it shows no badge, never "In process", which is only the default. It names the kind in plain words ("In process", "Worker thread", "Child process"), a custom target by its `name` ("Custom" without one), and a kind this version does not know by its raw string |
| Worker page Target card | the Worker page's needs, and `workers.list` on the queue's answer, as its instances. An instance too old to report `target` shows "—" and a line saying it predates target reporting. The file path only when the API sends it (`serialize.exposeProcessorFiles`), with nothing marking one withheld. Instances reporting different targets get one row per target, with the instances reporting each |
| Workers table summon badge (the Workers page, a queue's Workers panel and a worker page's Instances table) | the worker reports `summon`; a worker reporting none shows no badge, never a "not summoned", since a worker too old to say sends no `summon` either. It reads "Summoned by" and the summoner's kind ("Summoned by ecs"), or "Summoned" when the summoner gave none, and its tooltip gives the attempt's id and the mode and deadline the summoner requested, each called a request, with a mode this version does not know as its raw string |
| Worker page Summoned card | the Worker page's needs, and `workers.list` on the queue's answer, as its instances. Only when an instance reports `summon`, one row per such instance: its summon id, the summoner's kind, and the mode and deadline the summoner requested, each column only when an instance has a value. The handle only when the API sends it (`serialize.exposeSummonHandles`), with nothing marking one withheld. The instances reporting no summon are counted, without saying why |
| Workers page server table pager | the Workers nav entry's needs, and a server section holds more workers than one page (25). One pager per server section, the table it pages: a pager over the whole page would have to cross the host headings that say where a worker runs. The section's own worker count is the server's, not the page's |
| Queue Workers panel pager | the Workers panel's needs, and the queue has more live workers than one page holds (25) |
| Worker page Instances pager | the Worker page's needs, and the key has more live instances than one page holds (10) |
...
- **Status is the lifecycle, not activity.** A local runner's badge is its
  ...
  item's `isRunning`). A runner registered in another process has no
  lifecycle status here: the list and its screen show Paused or Active from
  the shared `isPaused` flag.
- **The history includes the run in flight.** A run that has started and not
  finished is the newest row, with status `running`.
- **A runner registered in another process** is marked "Other process".
  Pause, resume, reschedule and trigger still work, through the shared state,
  and the owner adopts them at its next sync; the screen says so. A trigger
  for such a runner always goes through the trigger queue, whatever its
  `queueRuns` says, so it comes back `queued` with its position, or `skipped` with
...
their interval either way: the Overview sparklines, the workers, demand and
...
| Workers panel | 10 s |
| Demand panel | 10 s |
| Overview Demand column | 5 s, for the page on screen |
...
- Runners: `runners-list`, `runners-count`, `runner-row-<id>`,
  ...
  `no-active-runs`, `run-<runId>`, `history-row-<runId>`,
  `history-open-log-note` (the `logs=` run is not on this page),
  `non-local-note`, `runner-hidden`, `runner-not-found`.
...
- Runner actions: the buttons sit in a `role="group"` named
  `Runner actions`. Inside it or its dialogs: `runner-non-local-hint`,
  `trigger-non-local-note`, `kill-waiting`, and `schedule-next-run` in the
