...
| `/runners` | Every runner in the namespace (`GET /runners`): local ones first, with their name and status, then remote ones by id. Filtered by id or name in the browser, then paged in the browser too, 25 to a page, with the window in the URL (`offset`, `limit`) as on `/queues`; changing the filter restarts it, and a page's worth of runners or fewer shows no pager. Each row links to its runner. |
...
- Runners: `runners-list`, `runners-count`, `runner-row-<id>`,
  ...
  `remote-note`, `runner-hidden`, `runner-not-found`.
