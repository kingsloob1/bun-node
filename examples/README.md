...
| [`@kingsleyweb/bun-jobs-ui`](../packages/bun-jobs-ui) | [`bun-jobs-ui/`](./bun-jobs-ui) | 9 examples: the management UI beside the management API — quick start, the shell, bundle and security headers checked without a socket, a cross-origin API, the queue, runner, Events and API docs screens' deep links and the permissions they are drawn from, a live seeded demo of queues and runners to click through, a browser pausing a queue, retrying a job and pausing a runner, a browser reading the API docs and trying their operations, and a browser watching the live badge and the Events console — plus an option tour; each asserts what it shows |
