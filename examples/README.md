...
| [`@kingsleyweb/bun-jobs-ui`](../packages/bun-jobs-ui) | [`bun-jobs-ui/`](./bun-jobs-ui) | 7 examples: the management UI beside the management API — quick start, the shell, bundle and security headers checked without a socket, a cross-origin API, the queue and runner screens' deep links and the permissions they are drawn from, a seeded demo of queues and runners to click through, and a browser pausing a queue, retrying a job and pausing a runner — plus an option tour; each asserts what it shows |
