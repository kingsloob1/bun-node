...
| [`remote-control.ts`](./07-runner/remote-control.ts) | `controller(id)` from a process that registered nothing: pause, reschedule, resume and trigger a runner another **process** owns ([`helpers/runner-owner.ts`](./07-runner/helpers/runner-owner.ts)); `info`, `history`, `stats`; no remote kill |
