...
| [`execution-modes.ts`](./07-runner/execution-modes.ts) | one handler in `in-process`, `worker-thread` and `child-process`, with what differs |
...
| [`worker-targets.ts`](./10-options/worker-targets.ts) | `target` in each local kind, each taking only its own tuning (another kind's is refused), and `BUN_JOBS_MODE` in the target's own word; what works inside an isolated job and what does not (`schedule`, `update`, `disable` and `enable` included); `job.fail()` in a child process and a `Worker`, reported when the attempt settles; an awaited `updateProgress` is in the store before the completion is |
| [`job-methods.ts`](./10-options/job-methods.ts) | `job.fail()` inside a processor (dead whatever attempts are left, the reason winning over a later throw, the job's `deadLetter`, no `cause` for a string and the `Error` as the cause) and from outside (a pending job buried at once; an active one buried under its lock, its worker aborting at its next heartbeat, `failed` and `dead` exactly once, locally and published, filed only in its own `deadLetter`); `schedule()`, `update()` with each field and `onlyIn`, `this \| null` for a job that is gone; `disable()` / `enable()` on an occurrence and a `ConfigError` on a one-off; `disableRepeatable()` / `enableRepeatable()` — `listRepeatables()` `disabled`, `nextJobId` / `nextRunAt` `null` while disabled, the pending occurrence removed, the one in flight finishing, enable scheduling from now with no backfill, `add({ repeat })` not re-enabling (it replaces the definition — interval, `tz`, `limit`, payload, options — but adds no occurrence and emits or publishes nothing, answering `wasAdded: false`; enable then schedules from the new definition), `removeRepeatable()` clearing the flag; `remove()` / `promote()` / `retry()` emitting and publishing; `progress` as `RunProgress \| null`; `extendLock()` / `touch()` only from the processor's view |
| [`runner-options.ts`](./10-options/runner-options.ts) | every `BunRunnerOptions` field, `RunContext`, every runner method and event, `BunRunnerManager` and `controller()`; `spawn.stdout`/`stderr` default to `"pipe"` while `captureLogs` is on, `"inherit"` with it off; `control` `"auto"` |
| [`bunjobs-options.ts`](./10-options/bunjobs-options.ts) | every `BunJobsOptions` field and `BunJobs` method, `jobsFromContext` |
| [`draft-and-process-every.ts`](./10-options/draft-and-process-every.ts) | every `JobDraft` member and `RepeatEveryOptions` field, the series setters (`limit`, `tz`, `endingAt`, `catchUp`, `immediately`), their guard (on `JobBuilder.limit()` too) and the values they refuse, a time zone checked however it is given (`tz()`, `repeatEvery(…, { tz })`, `withOptions({ repeat })`, `""` refused), date phrases read at `save()`, precedence, saving twice, refused combinations; `processEvery` (option and method), `processEveryMs`, and a worker's runtime `pollInterval` / `maxBlock`: limits, precedence, waits in progress per driver, the promotion sweep, pause, a failed `run()`, Redis `maxBlockSeconds`; and an idle worker's remembered next-due time: its empty passes stop promoting while it stands, a job another **driver instance** schedules earlier runs within one promotion sweep (`min(pollInterval, 1s)`, the measured lateness printed), and the worker's own instance is not held back by it |
| [`read-apis.ts`](./10-options/read-apis.ts) | every `ListJobsOptions` field; `search` taken literally and folded for case per engine; `page` totals; `getJobs` order, gaps and repeats; `listWorkers` fields, `reportInterval` and a lapsed record; `getThroughput` bounds, buckets and retried failures; `getQueueSummaries`; each driver fallback; the heartbeat's `rssBytes` and `heartbeatRttMs` — the process's memory (two workers in one process, the same figure, and a fleet totalled one row per host+pid) and the previous write's driver round trip, absent on a first record and on an older one; `completed` / `failed` per incarnation across a restart while the queue's totals keep counting |
| [`notifier.ts`](./10-options/notifier.ts) | every `JobsNotifierOptions` field and member — `hold()` / `unfollow()` reference-counted and `following` listing only live subscriptions among them — every published queue and runner event and its payload, `retried` from a single `retry()` included; `on("followed")` with its `NotifierFollowSource` (`discovery` / `follow`, never for a `hold`) and `followedForGood(kind)` |
| [`driver-options.ts`](./10-options/driver-options.ts) | every option of every driver and connection helper; the JSON-safe options a `DriverConfig` carries (`pollInterval`, `eventRetentionMs`, `maxBlockSeconds`, `clientOptions`), checked after a trip through JSON; each driver's `capabilities` (`jobAttribution` included); every SQL table and Mongo collection by name, run logs and analytics included; reading an unknown runner (or peeking or popping its empty trigger queue) registers nothing; `peekQueuedTrigger` returns the head a pop takes next, `force` included, without removing it; a queued trigger's `force`; a Redis wake an abandoned wait took; server sections run when their URL is set |
| [`errors.ts`](./10-options/errors.ts) | every error class, triggered through the public API, with its `code` and fields; `NotSupportedError` from every feature that needs a missing driver method, its `needs` naming the call (`update()`, `add({ debounce })`, `fail()`, `disable()`, `disableRepeatable()`, …) — the queue's repeat controls refusing even an unknown key rather than answering `false`; `UnrecoverableJobError`'s `{ cause }` option, and the cause surviving storage |
...
| [`jobs-api-options.ts`](./10-options/jobs-api-options.ts) | every `createJobsApi` option: each `ConfigError` it refuses construction with, `mode` / `readOnly` / `actions` (an allow-list with six opt-ins — `jobs.add`, `jobs.update`, `queues.defaults`, `queues.applyDefaults`, `workers.configure`, `runners.configure` — whose nine routes are absent by default; naming only two of them turns everything else off) / driver-capability pruning cross-checked against `/meta` and `/meta/permissions`, route counts that follow the backend's `addedByState`, the job-log path pruned from the OpenAPI document only when both reading and clearing are gone, RFC 9457 problems, `authorize` asked exactly once (a request failing a check — body, query, JSON, CSRF — still asked with the queue / job / runner its path names, and `ctx.route`; only an invalid path untargeted) and the resulting status matrix on a body, a query, a runner and a CSRF failure (allow-all 400, refusing untargeted still 400, refusing that queue or runner 403 `FORBIDDEN`; `CSRF_REJECTED` is itself 403), `limits`, `cors` / `csrf` / `trustProxy`, `serialize`, `addableNames`, `runnerTriggerArgs`, `validateResponses`, `docs`, and what a client is told: `/meta` `csrf` / `limits` (with the derived `defaultClean` and `maxRetryAllIds`, and `maxApplyDefaults` — 1000 by default, above 10000 refused) / `addableNames` / `runnerTriggerArgs` / `websocket.port`, `api.info`, `/meta/permissions?channel=` (its `key` whenever the channel parsed, refused or not), `POST /queues/:queue/jobs` creating a new queue (and still 403 for a name that is not addable, 404 outside a configured `queues` list), a stale `queueCacheMs` cache re-checked once per window before a 404, `/queues` paging and case-insensitive search, `/overview` `throughputSeries`, the 191 / 1024 job-id caps, 400 `INVALID_ARGUMENT` for an id `assertJobId` refuses (a control character, a leading `.`, a lone surrogate), `/meta` `publishing` (true / false) and the socket warning given only when it is false, the CSRF header and JSON rule in the OpenAPI document, the contract's constants; the fail, disable and enable routes by name (default-on mutations of the jobs half), `POST …/jobs/:id/fail` (200, 409, 404, 400 — a whitespace-only `reason` included — a padded reason stored exactly as sent, an active job with `failed` and `dead` published exactly once), `POST …/repeatables/:key/disable` and `…/enable` (idempotent, 404 for an unknown key) and `RepeatableDto.disabled`; `/meta/permissions` costing N + 1 `authorize` calls (N + 2 with a `channel`), each preview carrying the `ctx.route` its real request carries (so a route-based `authorize` answers the map as it answers the request) and `PermissionsDto.actions` typed `Partial<Record<JobsApiAction, boolean>>` (a compile-time check); `listQueues: "authorized"` hiding denied queues from `/queues` (paging counts only what is shown) and `/overview`, one `queues.read` per queue, a 500 for a throwing `authorize`, and the `ConfigError` without `queues.read`; `GET /runners` items' `isLocal` / `isPaused` / `isRunning` for local and remote runners (matching each runner's own read), lifecycle `status` (`idle` = not started, `running` = armed, not a run in flight), and `local` as a copy of `isLocal`, `deprecated: true` in the OpenAPI document and `@deprecated` in the contract; `PUT /runners/:runner/schedule` 400 `INVALID_SCHEDULE` `issues` naming `schedule`, `schedule.cron` or `schedule.tz` (a bad cron before a bad zone), while an interval below 1 or an `anchor` / `at` past the contract's `MAX_DATE_MS` is 400 `VALIDATION` at that field, as is a job's `runAt` past it (`PATCH` and `POST …/jobs`); `rssBytes` and `heartbeatRttMs` on all three worker reads, omitted rather than zeroed on a record without them, and typed optional on the contract |
...
| [`job-defaults.ts`](./10-options/job-defaults.ts) | queue job defaults: get/set/reset of the stored override (merge patch, `null`, rising `seq`, `expectedSeq` → `contended`), precedence over `define()` and `defaultJobOptions`, propagation and `jobDefaultsRefreshInterval`, `opts.explicit`; the `applyJobDefaults()` walk (cursor paging, each cursor bound to its walk — queue, `seq`, states and keys — `dryRun`, `states`/`keys`, `skippedExplicit`/`skippedUnmarked`/`includeUnmarked`, `exhausted`, priority reorder, `JobDefaultsChangedError`, irreversibility, repeat series); the four API routes with the opt-in `queues.defaults`/`queues.applyDefaults`, `readOnly`, `/meta.features.jobDefaults`/`jobDefaultsApply` (false in `runner` mode, pruned by a driver without support), `limits.maxApplyDefaults`, `OPERATION_IN_PROGRESS` |
| [`analytics-and-attribution.ts`](./10-options/analytics-and-attribution.ts) | the `metrics` option (defaults, caps, the driver config's `metrics`, merge field by field, `workers` / `runners: false`), what is recorded for queues, workers and runners (failed attempts vs the failed state, timeouts in `timeout` only, durations, busyness, heartbeat totals), range resolution against a fixed `now` (`to` the last bucket's start, `end` exclusive, every clamp `reason`, `RANGE_NOT_RETAINED`, `VALIDATION` / `INVALID_ARGUMENT` / `BULK_LIMIT`), every analytics route and `/overview`'s `analytics` block, analytics pruned on a driver without them; `processedBy` (the last attempt's worker and key, kept through settle; `workerId` clearing on settle is not new), the `workerKey` / `workerId` / `finishedFrom` / `finishedTo` filters in `list()` / `page()` and on the API, `exposeHosts`, the 100-value API cap, SQL attribution live until `syncSchema()`; `countAdded()` and `sort: "createdAt"` with `features.addedByState` on memory, SQL and MongoDB, and the refusals, pruned routes and scan fallback on file and Redis |
| [`run-logs-and-clears.ts`](./10-options/run-logs-and-clears.ts) | run-log capture per execution mode (which stream each source lands on), console attribution, the `logs` hint (`{ runId, lastSeq }`, forward-only, throttled, final seq), caps (`maxLines` / `maxLineBytes` / `maxBytes`, `dropped`, `truncated`, `capped` live-only), redaction (defaults, keys, patterns, off), job log clears (409 `JOB_ACTIVE`, `removed`, the route gone under `readOnly` or an `actions` list without `jobs.clearLogs`) and runner history clears (preview, live run kept, crashed record removed, remote clear, `staleAfter` bounds), on every driver |
| [`cross-process-control.ts`](./10-options/cross-process-control.ts) | `WorkerController` pause / resume / stop by key or id, `WorkerStateConflictError` on a stopped worker, `setConfig` validation / merge / `contended` / `null` / reset, stop persistence (`"process"` vs `"key"`, overridable; a replacement stays stopped under a key-persisted stop), the API's 409s, runner config changes announced to other owners at once (`configured`), `RunnerController.updateConfig`, `CONFIG_NOT_ALLOWED`, a driver-instance runner allowing `in-process` only; a worker's first-start `state` (no `previous`: running, paused before its `run()`, or stopped under a key stop) and `previous` on every later change; runner `config.error.keys` for a partial and a whole refusal, and `keys: []` on a pre-`keys` stored error; and a buried flow completing whichever order it is retried in; a runner another context registered found at once over `GET /runners/{id}` — the cached discovery re-reads on a miss, at most once per cache window |
...
| [`live-events-delivery.ts`](./11-management-api/live-events-delivery.ts) | what the socket delivers, on any backend: the first events of a queue or runner nothing has discovered, `retried` / `cleaned` on each job's channel, and `all` / `queues` / `runners` hiding a queue or runner the host denies; the broad `workers` channel following new queues — in-process with no gap, another process's queue with one `queue-discovered` gap scoped to `workers` — and `GET /workers` (with `/analytics/workers`) listing a just-created queue's worker despite the queue cache |
...
| [`demand-for-a-scaler.ts`](./11-management-api/demand-for-a-scaler.ts) | the depth endpoint for a scaler: a read-only API with `queues.read` and `queues.list` alone and a token of its own; `GET /queues/{queue}/demand` matching `getDemand()`, a paused queue at 0, 403 on a queue `authorize` denies and 404 on one that does not exist, `?cap` not taken; `GET /demand` under `listQueues: "authorized"`, `?queues=` keeping its order and dropping hidden and unknown names; the same figures as Prometheus text by `?format=prometheus` and by `Accept` |
