...
| [`runner-options.ts`](./10-options/runner-options.ts) | every `BunRunnerOptions` field, `RunContext`, every runner method and event, `BunRunnerManager` and `remote()`; `spawn.stdout`/`stderr` default to `"pipe"` while `captureLogs` is on, `"inherit"` with it off; `remoteControl` `"auto"` |
