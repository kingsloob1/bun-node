...
| [`worker-targets.ts`](./10-options/worker-targets.ts) | `target` in each local kind, each taking only its own tuning (another kind's is refused), and `BUN_JOBS_MODE` still in the runner's spelling; what works inside an isolated job and what does not (`schedule`, `update`, `disable` and `enable` included); `job.fail()` in a child process and a `Worker`, reported when the attempt settles; an awaited `updateProgress` is in the store before the completion is |
